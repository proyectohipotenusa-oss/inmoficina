...
import { 
  ...
  Home, Phone, Mail, Users, PenTool, ChevronLeft, ChevronRight, Building2, User
} from 'lucide-react';
...
export default function Agenda() {
  ...
  const toggleCompletada = async (tarea: any, e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  // Generar tira de días (7 días atrás, 14 adelante)
  const stripDays = useMemo(() => {
    const days = [];
    const d = new Date();
    d.setDate(d.getDate() - 3); // Empezar 3 días atrás
    for(let i=0; i<14; i++) {
      days.push({
  ...
  return (
    <Layout title="Agenda">
      ...
      {/* STRIP HORIZONTAL DE FECHAS (VIP) */}
          ...
          return (
            <button 
              ...
              className={`flex-shrink-0 w-16 h-20 rounded-2xl flex flex-col items-center justify-center gap-1 transition-all duration-300 border ${
                isSelected 
                  ? 'bg-brand-500 border-brand-400 shadow-[0_0_20px_rgba(59,130,246,0.3)] text-white scale-105' 
                  : day.isToday 
                    ? 'bg-white/10 border-white/20 text-white hover:bg-white/15'
                    : 'bg-ink-900 border-white/5 text-white/50 hover:bg-ink-800 hover:text-white/80'
              }`}
            >
      ...
      {loading ? <div className="card py-24 flex items-center justify-center text-white/40"><Loader2 className="animate-spin" size={22} /></div> : (
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          ...
          {/* COLUMNA PRINCIPAL: Tareas del Día */}
          ...
          {/* COLUMNA LATERAL: Atrasadas y Alertas */}
...
// COMPONENTE DE TARJETA DE TAREA (VIP)
function TaskCard({ t, leads, props, onEdit, onToggle }: { t: any, leads: any[], props: any[], onEdit: any, onToggle: any }) {
    ...
    <div className={`card p-4 flex items-start gap-4 group border-white/5 hover:border-white/10 transition cursor-pointer ${t.completada ? 'opacity-50 grayscale-[0.5]' : ''}`} onClick={onEdit}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1">
          <span className={`px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider ${tipoInfo.bg} ${tipoInfo.color}`}>
            {t.tipo}
          </span>
          <span className="text-[11px] text-white/50 font-medium flex items-center gap-1"><Clock size={11} /> {t.hora || 'Todo el día'}</span>
        </div>
        
        <div className={`text-sm font-bold truncate mb-2 ${t.completada ? 'text-white/60 line-through' : 'text-white'}`}>
          {t.titulo}
        </div>
          ...
          <div className="flex flex-wrap gap-2 mt-2">
            {lead && (
              <div className="flex items-center gap-1.5 px-2 py-1 rounded-md bg-white/5 border border-white/5 text-[10px] text-white/70">
                <User size={10} className="text-brand-400"/> <span className="truncate max-w-[120px]">{lead.nombre}</span>
              </div>
            )}
            {propiedad && (
              <div className="flex items-center gap-1.5 px-2 py-1 rounded-md bg-white/5 border border-white/5 text-[10px] text-white/70">
                <Building2 size={10} className="text-amber-400"/> <span className="truncate max-w-[120px]">{propiedad.referencia || 'Propiedad'}</span>
              </div>
            )}
          </div>
        )}
...
// DIÁLOGO CON SELECTORES VIP
function TareaDialog({ tarea, leads, propiedades, onClose, onSaved, selectedDate }: { tarea?: any, leads: any[], propiedades: any[], onClose: () => void, onSaved: () => void, selectedDate: string }) {
  const { perfil } = useAuth();
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    const payload = { 
      ...
      tipo, titulo, fecha, hora, estado_tarea: estado, descripcion, completada,
      lead_id: leadId || null,
      propiedad_id: propId || null
    };

    const res = isEditing ? await supabase.from('tareas').update(payload).eq('id', tarea.id) : await supabase.from('tareas').insert(payload);
      ...
      <div className="relative w-full max-w-lg card p-0 overflow-hidden animate-slide-up bg-ink-950 border-white/10 shadow-2xl">
        ...
        <form id="tarea-form" onSubmit={onSubmit} className="p-6 space-y-5 overflow-y-auto max-h-[70vh] custom-scrollbar">
            ...
            <div className="grid grid-cols-6 gap-2 mt-1">
              {TIPOS_TAREA.map((t) => (
                <button
                  ...
                  className={`flex flex-col items-center justify-center gap-1.5 p-2 rounded-xl border transition-all ${tipo === t.id ? `bg-brand-500/20 border-brand-500 ${t.color}` : 'bg-ink-900 border-white/5 text-white/40 hover:bg-white/5 hover:text-white'}`}
                >
                  <t.icon size={16} />
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="label">Cliente Asociado (Opcional)</label>
              <select className="input bg-ink-900 border-white/10 text-sm" value={leadId} onChange={e => setLeadId(e.target.value)}>
            ...
            <div>
              ...
              <select className="input bg-ink-900 border-white/10 text-sm" value={propId} onChange={e => setPropId(e.target.value)}>
                <option value="">Sin vincular</option>
                {propiedades.map(p => <option key={p.id} value={p.id}>{p.referencia || 'Propiedad'} - {p.titulo.substring(0,20)}...</option>)}
              </select>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select className="input bg-ink-900 border-white/10 text-sm" value={estado} onChange={(e) => setEstado(e.target.value)}>
