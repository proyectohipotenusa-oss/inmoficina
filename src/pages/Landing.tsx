...
export default function Landing() {
  ...
  const FAQS = [
    ...
    { q: "¿Realmente cerraré más ventas con Inmoficina?", a: "Sí. No somos un archivador de propiedades, somos una herramienta de persuasión. Cada informe que envías está diseñado para generar autoridad y confianza inmediata en el cliente." },
          ...
          <div className="relative aspect-[4/5] rounded-[1.5rem] overflow-hidden border border-white/10 shadow-2xl bg-ink-900">
            ...
            <div className="absolute bottom-6 left-6 right-6">
              <div className="p-4 bg-ink-950/80 backdrop-blur-md rounded-xl border border-white/10 flex items-center gap-3">
                ...
                <p className="text-sm font-medium text-white/90">Tu identidad en el centro de todo.</p>
              </div>
            </div>
          </div>
            ...
            {contactStatus === 'success' ? (
              ...
              <form onSubmit={handleContactSubmit} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div><label className="label-xs">Teléfono</label><input type="tel" className="input bg-ink-950 border-white/5 text-xs px-3 py-2 w-full rounded-lg" value={contactData.telefono} onChange={e => setContactData({...contactData, telefono: e.target.value})} /></div>
                </div>
                <div><label className="label-xs">Mensaje *</label><textarea required rows={3} className="input bg-ink-950 border-white/5 text-xs px-3 py-2 w-full rounded-lg resize-none" value={contactData.mensaje} onChange={e => setContactData({...contactData, mensaje: e.target.value})} /></div>
